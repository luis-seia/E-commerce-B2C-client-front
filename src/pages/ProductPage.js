...
function ProductPage(props) {
    ...
    const ourRequest = useMemo(() => axios.CancelToken.source(), []);
    ...
    useEffect(() => {
        const gatData = async () => {
          ...
          try {
              ...
              setColor(data.data.color?.length >= 0 ? `#${data.data.color[0]}` : null);
              setsize(data.data.size?.length >= 0 ? data.data.size[0] : null);
              setIsLoading(false)  
      ...
      <Navbar/>
        {
            ...
            <InfoContainer>
                <TitleContainer>
                  ...
                  <Dno>Design Nr - {product.productno}</Dno>
                </TitleContainer>
                ...
                {product.quantity <= 5 && <Stock inStock={product.quantity >= 1}>{product.quantity >= 1 ? `Apenas ${product.quantity} restantes em estoque` : "Currently unavailable"}</Stock>}
                <FilterContainer>
                    <Filter>
